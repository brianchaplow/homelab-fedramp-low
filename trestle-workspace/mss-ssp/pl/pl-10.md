...
The FedRAMP Rev 5 Low baseline (156 controls) is selected and encoded as an OSCAL profile at `trestle-workspace/profiles/fedramp-rev5-low/profile.json` (UUID 512149a6-7f04-4c01-bb1b-78eafd6a950d, version 5.1.1+fedramp-20240111-0). The rationale for Low selection is documented in the whole-project design spec §2.1: FIPS 199 Low categorization (C: Low, I: Low, A: Low) because the Managed SOC Service processes only operational telemetry -- alert metadata, network flows, and vulnerability scan data -- with no PII, PHI, CUI, or financial data; the 99% monthly availability target is advisory rather than safety-critical. The selected baseline is implemented as an OSCAL profile that imports the NIST SP 800-53 Rev 5 catalog (`trestle-workspace/catalogs/nist-800-53-rev5/catalog.json`, 10.7 MB). The resolved catalog at `trestle-workspace/catalogs/fedramp-rev5-low-resolved/catalog.json` contains exactly 156 controls (verified by `trestle validate` per ADR 0007 done criteria). The derivation story for the profile -- including the GSA/fedramp-automation repo deletion and the compliance-trestle-fedramp plugin sourcing path -- is documented in ADR 0006 Deviation 11 and `oscal/profile/SOURCE.md`.
